<?php
/**
 *
 * @category        modules
 * @package         news_img
 * @author          WBCE Community
 * @copyright       2004-2009, Ryan Djurovich
 * @copyright       2009-2010, Website Baker Org. e.V.
 * @copyright       2019-, WBCE Community
 * @link            https://www.wbce.org/
 * @license         http://www.gnu.org/licenses/gpl.html
 * @platform        WBCE
 *
 */

// Description du module
$module_description = "Module de cr&eacute;ation de nouvelles avec une image et une galerie d'articles (facultatif).";

// Variables pour le backend
$MOD_NEWS_IMG['ACTION'] = "Entr&eacute;es s&eacute;lectionn&eacute;es";
$MOD_NEWS_IMG['ACTIVATE'] = "activer";
$MOD_NEWS_IMG['ACTIVATE_POST'] = "activer le post";
$MOD_NEWS_IMG['ADD_GROUP'] = 'Ajouter un groupe';
$MOD_NEWS_IMG['ADD_POST'] = 'Ajouter un message';
$MOD_NEWS_IMG['ADD_TAG'] = 'Ajouter un mot clé';
$MOD_NEWS_IMG['ADVANCED_SETTINGS'] = 'Param�tres avanc�s';
$MOD_NEWS_IMG['ALL'] = "Tous";
$MOD_NEWS_IMG['ASSIGN_GROUP'] = "Attribuer un groupe";
$MOD_NEWS_IMG['ASSIGN_TAGS'] = "Attribuer des balises"; 
$MOD_NEWS_IMG['CLEAR_PUBLISHED_UNTIL'] = "effacer la date d'expiration" ;
$MOD_NEWS_IMG['CLEAR_PUBLISHED_WHEN'] = "r�initialiser la date de publication";
$MOD_NEWS_IMG['CONTINUE'] = "Suivant";
$MOD_NEWS_IMG['COPY'] = "copier";
$MOD_NEWS_IMG['COPY_WITH_TAGS'] = 'copie (y compris les balises)';
$MOD_NEWS_IMG['COPY_POST'] = 'Copier le message';
$MOD_NEWS_IMG['CURRENT_SECTION'] = 'Section actuelle';
$MOD_NEWS_IMG['DEACTIVATE'] = "d&eacute;sactiver";
$MOD_NEWS_IMG['DEACTIVATE_POST'] = "d&eacute;sactiver le post";
$MOD_NEWS_IMG['REFRESH_ACCESS_FILES'] = "R&eacute;g&eacute;n&eacute;rer les fichiers d'acc&egrave;s";
$MOD_NEWS_IMG['REFRESH_ACCESS_FILES_CONFIRM'] = "R&eacute;&eacute;crire les fichiers d'acc&egrave;s de tous les billets de cette section ?";
$MOD_NEWS_IMG['REFRESH_ACCESS_FILES_TOOLTIP'] = "Recr&eacute;e les fichiers d'acc&egrave;s pour tous les billets de cette section — utile apr&egrave;s un import ou en cas de liens cass&eacute;s.";
$MOD_NEWS_IMG['REFRESH_ACCESS_FILES_SUCCESS'] = "%d fichier(s) d'acc&egrave;s r&eacute;g&eacute;n&eacute;r&eacute;(s)";
$MOD_NEWS_IMG['REFRESH_ACCESS_FILES_FAILED'] = "%d fichier(s) d'acc&egrave;s n'ont pas pu &ecirc;tre r&eacute;g&eacute;n&eacute;r&eacute;s";
$MOD_NEWS_IMG['DEMODATA_IMPORT'] = 'Importer des donn&eacute;es de d&eacute;monstration';
$MOD_NEWS_IMG['DEMODATA_IMPORT_INFO'] = "Cette section est encore vide. Vous pouvez importer ici des billets d'exemple pour tester la mise en page. L'option dispara&icirc;t d&egrave;s que la section contient ses propres billets ou groupes.";
$MOD_NEWS_IMG['DEMODATA_PACK'] = 'Lot de donn&eacute;es';
$MOD_NEWS_IMG['DEMODATA_IMPORT_BUTTON'] = 'Importer';
$MOD_NEWS_IMG['DEMODATA_CONFIRM'] = 'Vraiment importer les donn&eacute;es de d&eacute;monstration dans cette section ?';
$MOD_NEWS_IMG['DEMODATA_IMPORT_SUCCESS'] = 'Donn&eacute;es de d&eacute;monstration import&eacute;es';
$MOD_NEWS_IMG['DEMODATA_IMPORT_FAILED'] = "&Eacute;chec de l'import des donn&eacute;es de d&eacute;monstration";
$MOD_NEWS_IMG['DEFAULT_PREVIEW_IMAGE'] = 'Image par d&eacute;faut';
$MOD_NEWS_IMG['DEFAULT_PREVIEW_IMAGE_NONE_SET'] = "aucune image par d&eacute;faut d&eacute;finie";
$MOD_NEWS_IMG['DEFAULT_PREVIEW_IMAGE_UPLOAD'] = "T&eacute;l&eacute;verser une nouvelle image";
$MOD_NEWS_IMG['DEFAULT_PREVIEW_IMAGE_PICK_EXISTING'] = "choisir parmi les images de billets existantes";
$MOD_NEWS_IMG['DEFAULT_PREVIEW_IMAGE_REMOVE'] = "supprimer";
$MOD_NEWS_IMG['DEFAULT_PREVIEW_IMAGE_SEARCH'] = "rechercher...";
$MOD_NEWS_IMG['NO_IMAGES_AVAILABLE'] = "Aucune image n'a encore &eacute;t&eacute; t&eacute;l&eacute;vers&eacute;e pour cette section.";
$MOD_NEWS_IMG['DELETE'] = "effacer";
$MOD_NEWS_IMG['DELETEIMAGE'] = 'Supprimer cette image?';
$MOD_NEWS_IMG['DESCENDING'] = 'absent�';
$MOD_NEWS_IMG['EXPERT_MODE'] = 'mode expert';
$MOD_NEWS_IMG['EXPIRED_NOTE'] = "La publication n'est plus affich&eacute;e dans le front-end car la date d'expiration est d&eacute;pass&eacute;e.";
$MOD_NEWS_IMG['FIRST_EXPIRING_FIRST'] = 'premier expirant en premier';
$MOD_NEWS_IMG['FIRST_EXPIRING_LAST'] = 'premier expirant en dernier';
$MOD_NEWS_IMG['GALLERY_SETTINGS'] = "Param&egrave;tres de la galerie / de l'image";
$MOD_NEWS_IMG['GALLERYIMAGES'] = 'Images de la galerie';
$MOD_NEWS_IMG['GENERIC_IMAGE_ERROR'] = 'Probl&egrave;mes avec les images de publication et / ou de galerie. Veuillez v&eacute;rifier le nom du fichier, le type de fichier et la taille du fichier.';
$MOD_NEWS_IMG['GLOBAL'] = 'Mot clé global';
$MOD_NEWS_IMG['GOBACK'] = 'Retour';
$MOD_NEWS_IMG['GROUP'] = 'Groupe';
$MOD_NEWS_IMG['GROUPS'] = 'Groupes';
$MOD_NEWS_IMG['IMAGE_FILENAME_ERROR'] = 'Le nom du fichier est trop long (maximum 256 caract&egrave;res autoris&eacute;s)';
$MOD_NEWS_IMG['IMAGE_INVALID_TYPE'] = "Type d'image non support&eacute";
$MOD_NEWS_IMG['IMAGE_LARGER_THAN'] = "L'image est trop grande, max. Taille: ";
$MOD_NEWS_IMG['IMAGE_TOO_SMALL'] = "L'image est trop petite";
$MOD_NEWS_IMG['IMAGEUPLOAD'] = 'T&eacute;l&eacute;charger des images';
$MOD_NEWS_IMG['IMPORT_OPTIONS'] = "Importer les param&egrave;tres";
$MOD_NEWS_IMG['INFO_GLOBAL'] = "Les mots clés globaux s'appliquent à toutes les sections.";
$MOD_NEWS_IMG['INFO_RELOAD_PAGE'] = "La page se recharge; toutes les modifications non enregistrées seront perdues!";
$MOD_NEWS_IMG['LINK'] = 'Lien';
$MOD_NEWS_IMG['LOAD_VALUES'] = "Charger les valeurs";
$MOD_NEWS_IMG['MANAGE_POSTS'] = "G&eaucte;rer les post";
$MOD_NEWS_IMG['MOVE'] = "d&eacute;placer";
$MOD_NEWS_IMG['MOVE_WITH_TAGS'] = 'd�placer (y compris les balises)';
$MOD_NEWS_IMG['NEW_POST'] = 'Cr&eacute;er un nouveau message';
$MOD_NEWS_IMG['NEWEST_FIRST'] = 'le plus r�cent en haut';
$MOD_NEWS_IMG['NONE'] = "Aucun";
$MOD_NEWS_IMG['OPTIONS'] = 'Options';
$MOD_NEWS_IMG['OR'] = 'or';
$MOD_NEWS_IMG['ORDER_CUSTOM_INFO'] = 'Le param&egrave;tre &quot;custom&quot; permet le tri manuel des articles via les fl&egrave;ches haut / bas.';
$MOD_NEWS_IMG['ORDERBY'] = 'Trier par';
$MOD_NEWS_IMG['OVERVIEW_SETTINGS'] = 'Param&egrave;tres de la page de pr&eacute;sentation';
$MOD_NEWS_IMG['POST_ACTIVE'] = 'La publication est visible';
$MOD_NEWS_IMG['POST_CONTENT'] = 'Publier du contenu';
$MOD_NEWS_IMG['POST_INACTIVE'] = "La publication n'est pas visible";
$MOD_NEWS_IMG['POST_SETTINGS'] = 'Param&egrave;tres postaux';
$MOD_NEWS_IMG['POSTED_BY'] = 'Publi� par';
$MOD_NEWS_IMG['POSTS'] = 'Messages';
$MOD_NEWS_IMG['PREVIEWIMAGE'] = "Aper&ccedil;u de l'image";
$MOD_NEWS_IMG['SAVEGOBACK'] = 'Sauvegarder et revenir en arri&egrave;re';
$MOD_NEWS_IMG['SETTINGS'] = 'Param&egrave;tres de News';
$MOD_NEWS_IMG['SHOW_SETTINGS_ONLY_ADMINS'] = 'Param&egrave;tres seulement pour administrateurs';
$MOD_NEWS_IMG['TAG'] = 'Balise'; 
$MOD_NEWS_IMG['TAG_COLOR'] = 'Couleur de balise';
$MOD_NEWS_IMG['TAG_TEXT_COLOR'] = 'Couleur du texte';
$MOD_NEWS_IMG['TAG_EXISTS'] = 'Balise existe';
$MOD_NEWS_IMG['TAGS'] = 'Balises'; 
$MOD_NEWS_IMG['TAGS_INFO'] = 'Pour utiliser des mots clés, modifiez une publication et sélectionnez les mots clés souhaités.';
$MOD_NEWS_IMG['TO'] = "&agrave;";
$MOD_NEWS_IMG['UPLOAD'] = 'T�l�charger une nouvelle image';
$MOD_NEWS_IMG['USE_SECOND_BLOCK'] = 'Utiliser le deuxi�me bloc';
$MOD_NEWS_IMG['USE_SECOND_BLOCK_HINT'] = 'Doit �tre pris en charge par le mod�le';
$MOD_NEWS_IMG['VIEW'] = 'Pr�sentation / Vue';
$MOD_NEWS_IMG['VIEW_INFO'] = 'After changing the setting, hit save; the markups for post loop and post details view will be adjusted automatically.'; //

// parametres d'image
$MOD_NEWS_IMG['CROP'] = 'Recadrage';
$MOD_NEWS_IMG['GALLERY'] = "Galerie d'images";
$MOD_NEWS_IMG['GALLERY_INFO'] = 'Apr&egrave;s avoir modifi&eacute; les param&egrave;tres de la galerie, cliquez sur enregistrer; le balisage de la boucle image sera ajust&eacute; automatiquement.';
$MOD_NEWS_IMG['GALLERY_WARNING'] = "&Ecirc;tes-vous s&ucirc;r? Notez que les param&egrave;tres personnalis&eacute;s pour le balisage de la boucle d'image seront perdus.";
$MOD_NEWS_IMG['IMAGE_MAX_SIZE'] = "Max. taille de l'image en kilooctets ";
$MOD_NEWS_IMG['RESIZE_PREVIEW_IMAGE_TO'] = "Redimensionner l'image d'aper&ccedil;u en";
$MOD_NEWS_IMG['RESIZE_GALLERY_IMAGES_TO'] = 'Redimensionner les images de la galerie en';
$MOD_NEWS_IMG['TEXT_CROP'] = "Si le rapport de format de l'original ne correspond pas au rapport de format sp&eacute;cifi&eacute;, le chevauchement du bord le plus long sera supprim&eacute;.";
$MOD_NEWS_IMG['TEXT_DEFAULTS'] = 'Tailles par d&eacute;faut';
$MOD_NEWS_IMG['TEXT_DEFAULTS_CLICK'] = 'Cliquez pour choisir parmi les param&egrave;tres par d&eacute;faut';
$MOD_NEWS_IMG['THUMB_SIZE'] = 'Taille de la vignette';

// Uploader
$MOD_NEWS_IMG['DRAG_N_DROP_HERE'] = 'Glisser &amp; D&eacute;posez les fichiers ici';
$MOD_NEWS_IMG['CLICK_TO_ADD'] = 'Cliquez pour ajouter des fichiers';
$MOD_NEWS_IMG['NO_FILES_UPLOADED'] = 'Aucun fichier t&eacute;l&eacute;charg&eacute;.';
$MOD_NEWS_IMG['COMPLETE_MESSAGE'] = 'Enregistrez vos modifications pour afficher le t&eacute;l&eacute;chargement dans la galerie';

// Variables pour le frontend
$MOD_NEWS_IMG['PAGE_NOT_FOUND'] = 'Page non trouvée';
$MOD_NEWS_IMG['TEXT_AT'] = '&agrave;';
$MOD_NEWS_IMG['TEXT_BACK'] = 'Retour';
$MOD_NEWS_IMG['TEXT_BY'] = 'Par';
$MOD_NEWS_IMG['TEXT_LAST_CHANGED'] = 'Derni&egrave;re modification';
$MOD_NEWS_IMG['TEXT_NEXT_POST'] = 'Próximo post';
$MOD_NEWS_IMG['TEXT_O_CLOCK'] = 'horas';
$MOD_NEWS_IMG['TEXT_ON'] = '&agrave;';
$MOD_NEWS_IMG['TEXT_POSTED_BY'] = 'Publi&eacute; par';
$MOD_NEWS_IMG['TEXT_PREV_POST'] = 'En savoir plus';
$MOD_NEWS_IMG['TEXT_READ_MORE'] = 'Consulte mais informação';
$MOD_NEWS_IMG['TEXT_RESET'] = 'Redefinir';
$MOD_NEWS_IMG['TO'] = 'para';
$MOD_NEWS_IMG['IMPORT'] = 'importer';
